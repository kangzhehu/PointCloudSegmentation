#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2024/7/10 16:05
# @File    : S3DISDataLoader1.py.py
# @Description : tree 最邻近的
import os
import numpy as np
...
class S3DISDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0,
        ...
        for room_name in tqdm(rooms_split, total=len(rooms_split)):
            ...
            room_data = np.array(np.load(room_path))  # xyzrgbl, N*7
    ...
    def __getitem__(self, idx):
        ...
        tree = self.kd_trees[room_idx]

        pick_idx = np.random.choice(len(points), 1)
        center = points[pick_idx, :3].reshape(1, -1)
        selected_idx = tree.query(center, k=self.num_point)[1][0]
        # normalize
        selected_points = points[selected_idx, :]  # num_point * 6
        current_points = np.zeros((self.num_point, 9))  # num_point * 9
        current_points[:, 6] = selected_points[:, 0] / self.room_coord_max[room_idx][0]
        current_points[:, 7] = selected_points[:, 1] / self.room_coord_max[room_idx][1]
        current_points[:, 8] = selected_points[:, 2] / self.room_coord_max[room_idx][2]
        selected_points[:, 0] = selected_points[:, 0] - center[0][0]
        selected_points[:, 1] = selected_points[:, 1] - center[0][1]
        selected_points[:, 3:6] /= 255.0
        current_points[:, 0:6] = selected_points
        current_labels = labels[selected_idx]
