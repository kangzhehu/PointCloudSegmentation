...
def main(args):
    ...
    transforms = TRANSFORMS.Compose([
        ...
        TRANSFORMS.RandomTranslate(),
        TRANSFORMS.RandomShear(),
        TRANSFORMS.RandomFlip(),
        TRANSFORMS.RandomNoise(),
        TRANSFORMS.RandomDropout(),
        TRANSFORMS.NormalizeCoordinates()  # 最后进行归一化
    ])
    ...
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE, shuffle=True, num_workers=1,
                                                  ...
                                                  worker_init_fn=np.random.seed(int(time.time())))
    ...
    # classifier = MODEL.get_model(3, 512, NUM_CLASSES, 2, 2).cuda()
    radius_list = [0.1, 0.2, 0.4]
    num_samples_list = [32, 64, 128]
    embed_dim = 1024  # 需要匹配 SelfAttention 输入的 embed_dim
    num_heads = 8
    # classifier = MODEL.get_model(NUM_CLASSES, 1024, radius_list, num_samples_list, embed_dim, num_heads).cuda()
    classifier = MODEL.get_model(9, NUM_CLASSES, 16).cuda()
    criterion = MODEL.get_loss(weights).cuda()
    ...
    scheduler = WarmupCosineLR(
        optimizer,
        T_max=100 * len(trainDataLoader),
        eta_min=0.00001,
        warmup_steps=5 * len(trainDataLoader),
    )
    ...
    def test01(m):
        return bn_momentum_adjust(m, momentum)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for param_group in optimizer.param_groups:
            log_string(f"Current learning rate: {param_group['lr']}")
        momentum = MOMENTUM_ORIGINAL * (MOMENTUM_DECCAY ** (epoch // MOMENTUM_DECCAY_STEP))
        if momentum < 0.01:
            momentum = 0.01
        print('BN momentum updated to: %f' % momentum)
        # classifier = classifier.apply(lambda x: bn_momentum_adjust(x, momentum))
        classifier = classifier.apply(test01)
        num_batches = len(trainDataLoader)
        print_freq = np.max((num_batches // 10, 1))
        total_correct = 0
        total_seen = 0
        loss_sum = 0
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            assert points.shape[1] == 9, points.shape

            # seg_pred, trans_feat = classifier(points)
            # seg_pred = classifier(points[:, :3, :])
            ...
            # print("Loss at batch {}: {}".format(i, loss.item()))
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            loss_sum += loss
            # 如果当前批次是每个打印周期的最后一个批次,或者是整个数据集的最后一个批次,则执行打印指标信息的操作。
            if i % print_freq == print_freq - 1 or i == len(trainDataLoader) - 1 or i == 1:
                log_string('Training mean loss: %f' % (loss_sum / float(i)))
